...
`progarchives_artist` are the same band — that's your **dedup / join key**.

## How metadatarr consumes this

metadatarr ships a provider (`metadatarr/resolve/providers/progarchives.py`)
that imports this library, resolves a band for `PlaybackType.AUDIO` +
`"rock"`/`"progressive rock"` signals, and emits the external ids above plus an
`EntityRole.ARTIST` (group) entity. From that entity metadatarr derives a
...
`pyprogarchives` and `metadatarr` is enough; metadatarr auto-discovers the
