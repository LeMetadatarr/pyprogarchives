...
It scrapes the site's HTML into clean dataclasses. Browse the A-Z band index,
search by name, and fetch a band's full page: genre, country, biography, and
a rated discography. Every model exposes canonical ids for de-duplication and
an optional [metadatarr](../metadatarr) provider for cross-referencing.
...
> defaults to `curl_cffi` Chrome TLS impersonation (the `stealth` extra),
> which clears the bot check from most networks. From flagged IPs you may
> still hit a JS challenge. The best fix is to point it at a **FlareSolverr**
> instance for live data: `export PYPROGARCHIVES_FLARESOLVERR_URL=http://host:8191`.
> Or read from the **Internet Archive** with
> `PYPROGARCHIVES_TRANSPORT=wayback` (stale, but needs nothing). The parsing
> layer works the same regardless of how the HTML is fetched. See
...
An `ArtistDetail` carries `albums: List[Album]`. Each album has progarchives'
average rating, number of ratings, year, and cover.
...
This package is a pure scraper. It exposes progarchives' stable ids through
...
The metadatarr resolver consumes these ids. The `MetadataProvider` lives in
the [metadatarr](../metadatarr) repo (`metadatarr/resolve/providers/progarchives.py`),
